function out = testbench_swagFX
% TESTBENCH_SWAGFX Exercise audio plugin class
% to check for violations of plugin constraints and other errors.
%
% OUT = TESTBENCH_SWAGFX Return the output data from the
...
%
%   codegen testbench_swagFX    % Create the mex function
%   testbench_swagFX_mex        % Run the mex function
%
...
% validateAudioPlugin.
%
% Automatically generated by validateAudioPlugin 19-Nov-2020 22:28:16
...
for sampleRate = sampleRates
    ...
    for i = 1:numel(frameSizes)
        ...
        val = fromNormalizedswagS(mod(floor((i-1)./27),3)/2);
        plugin.swagS = val;
        paramState.swagS = val;
        checkForTampering(plugin, paramState, sampleRate, ...
            'Setting parameter ''swagS''');
...
if ~isequal(paramState.swagS, plugin.swagS)
    error('ValidateAudioPlugin:ParamChanged', ...
        '%s changed parameter ''swagS''.', cause);
end
...
val = 0.9 + (1.5-0.9)*normval;
end

function val = fromNormalizedswagS(normval)
enums = [ OutputSelect.cplxMag OutputSelect.realPart OutputSelect.imagPart ];
